...
Every prime power `q ≤ M` with exponent at least 2 has base at most `√M`, so the bases come from the
sieve indices below `(√M - 1) / 3`. The state holds a count of collected values in its low 64 bits,
the base's running power in the next 64, and the values themselves in `w`-bit fields above bit 128.
-/
...
/-- Perform `fuel` steps from sieve index `start`, collecting the powers of each index whose sieve
bit is set. -/
...
/-- Replace the starting state by a kernel-checked equal literal, the way `run_cert_hp` enters the
chain from the state holding the powers of 2 and 3. -/
