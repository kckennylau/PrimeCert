...
`Nat.Prime`. The argument runs in three steps:
...
theorem sieve_prime_set {n sqrtN t : ℕ} (ht1 : 1 ≤ t) (htM : t ≤ (n - 1) / 3)
...
theorem sieveLoopK_clears {M start t j m : ℕ} (hstart : 1 ≤ start)
  ...
  | succ f ih =>
    ...
    · have hje : j = start + f := by lia
      ...
      have hmask : (buildMaskK (num (start + f)) M ((num (start + f) * 5 - 1) / 3)
        ...
        rw [mask_iff (num (start + f)) M t num_mod6 hM ht]
        exact ⟨m, hm5, hm6, by grind⟩
      simp [hmask]

/-! ### Soundness number theory -/

theorem num_wheel {q : ℕ} (hq : q % 6 = 1 ∨ q % 6 = 5) : num ((q - 1) / 3) = q := by grind [num]
...
theorem coprime6_mod {m : ℕ} (h : Nat.Coprime m 6) : m % 6 = 1 ∨ m % 6 = 5 := by
...
theorem prime_ge5_mod6 {q : ℕ} (hq : q.Prime) (h5 : 5 ≤ q) : q % 6 = 1 ∨ q % 6 = 5 := by
...
is prime. -/
public theorem sieveK_testBit_iff (n sqrtN t : ℕ) (ht1 : 1 ≤ t) (htM : t ≤ (n - 1) / 3)
