...
public import PrimeCert.Meta.SmallPrime
...
meta def parsePrimePow (stx : TSyntax ``prime_pow) : Q(Nat) × ParsedPrimePow :=
  match stx with
...
meta def parseFactored (stx : TSyntax ``factored) : Q(Nat) × Array ParsedPrimePow :=
  match stx with
  | `(factored| $head * $body**) =>
